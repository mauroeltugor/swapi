...
const workOnCharacters = async (newUrl) => {
    try {
        const getCharacters = await people(newUrl)
...
workOnCharacters(url);

const load = document.getElementById("button")
let page = 2
load.addEventListener('click', async () => {
    await workOnCharacters(`https://swapi.dev/api/people/?page=${page}`);
    page ++;
});